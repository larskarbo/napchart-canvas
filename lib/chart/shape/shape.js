...
module.exports = function (Napchart) {
  ...
  function changeShape(chart) {
    // we are this shape:
    var currentShape = chart.oldShape
    var wantToShape = chart.data.shape
    ...
    } else if(currentShape == 'horizontalEllipse'){
      if(wantToShape == 'line'){
        shapeSequenze = ['circle', 'transitionShape', 'line']
      }
    } else if(currentShape == 'line'){
      if(wantToShape == 'horizontalEllipse'){
        shapeSequenze = ['transitionShape', 'circle', 'horizontalEllipse']
      } else if(wantToShape == 'circle'){
        shapeSequenze = ['transitionShape', 'circle']
      }
    }

    console.log(shapeSequenze)
    var index = 0
    function next(){

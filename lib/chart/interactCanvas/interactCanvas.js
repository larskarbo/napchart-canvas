...
module.exports = function (Napchart) {
  ...
  function hover (e, chart) {
    ...
    if(Object.keys(hit).length > 0){
      chart.setHover(hit.origin.id, hit.type)
    }else{
      chart.removeHovers()
    }
  ...
  function drag (e, chart) {
    ...
    originElement.duration = helpers.range(originElement.start, originElement.end)
  ...
  function getActiveElement (chart, identifier) {
    var activeElements = chart.activeElements
  ...
  function up (e, chart) {
    var identifier = findIdentifier(e)
    var element = getActiveElement(chart, identifier)
